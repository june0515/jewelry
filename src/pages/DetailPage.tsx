...
export function DetailPage(){
  ...
  return <section className="detail">
    ...
    <dl><dt>{t('wornTimes')}</dt><dd>{item.wearCount}</dd><dt>{t('lastWorn')}</dt><dd>{item.lastWornDate || t('notRecorded')}</dd><dt>{t('colors')}</dt><dd>{item.colors.join(' / ') || t('none')}</dd><dt>{t('occasions')}</dt><dd>{item.occasions.map(label).join(' / ') || t('none')}</dd><dt>{t('purchase')}</dt><dd>{item.purchaseDate || t('none')} {item.purchasePrice ? ` · $${item.purchasePrice}` : ''}</dd><dt>{t('source')}</dt><dd>{item.purchaseSource || t('none')}</dd><dt>{t('storageLocation')}</dt><dd>{item.storageLocation || t('none')}</dd><dt>{t('note')}</dt><dd>{item.note || t('none')}</dd></dl>
